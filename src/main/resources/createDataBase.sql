...
-- Schema faculty1
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `faculty1` DEFAULT CHARACTER SET utf8 ;
USE `faculty1` ;
...
CREATE TABLE IF NOT EXISTS `faculty1`.`human` (
  ...
  `patronymic` VARCHAR(25) NOT NULL COMMENT '',
  `role` ENUM('STUDENT','ADMIN','TEACHER') NOT NULL COMMENT '',
...
CREATE TABLE IF NOT EXISTS `faculty1`.`teacher` (
  `position` ENUM('ASSISTANT_PROFESSOR','ASSOCIATE_PROFESSOR','PROFESSOR') NOT NULL COMMENT '',
  `human_id` INT(11) NOT NULL COMMENT '',
  PRIMARY KEY (`human_id`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `faculty1`.`discipline` (
  ...
  `teacher_human_id` INT(11) NOT NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  UNIQUE INDEX `id_UNIQUE` (`id` ASC)  COMMENT '',
  UNIQUE INDEX `title_UNIQUE` (`title` ASC)  COMMENT '',
  INDEX `fk_discipline_teacher1_idx` (`teacher_human_id` ASC)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `faculty1`.`student` (
  `contract` TINYINT(1) NOT NULL COMMENT '',
  `human_id` INT(11) NOT NULL COMMENT '',
  PRIMARY KEY (`human_id`)  COMMENT '',
...
CREATE TABLE IF NOT EXISTS `faculty1`.`evaluation` (
  `id` INT(11) NOT NULL AUTO_INCREMENT COMMENT '',
  `discipline_id` INT(11) NOT NULL COMMENT '',
  `student_human_id` INT(11) NOT NULL COMMENT '',
  `statusInDiscipline` ENUM('DECLARED','REVOKED','CONFIRMED','DELETED') NOT NULL COMMENT '',
  `mark` ENUM('A','B','C','D','E','Fx','F') NULL DEFAULT NULL COMMENT '',
  `feedback` MEDIUMTEXT NULL DEFAULT NULL COMMENT '',
  PRIMARY KEY (`id`)  COMMENT '',
  UNIQUE INDEX `id_UNIQUE` (`id` ASC)  COMMENT '',
  INDEX `fk_evaluation_student1_idx` (`student_human_id` ASC)  COMMENT '',
  INDEX `fk_evaluation_discipline1_idx` (`discipline_id` ASC)  COMMENT '',
